      ...
      steps: [
        {
          ...
          description: '`ToPrimitive` je interna apstraktna operacija u JavaScript motoru (ECMA-262 §7.1.1) koja konvertuje objekat ili niz u primitivnu vrednost (string, broj ili simbol) kada god se objekat nađe u operaciji sa operatorima kao što su `+`, `==`, `<` ili `String()`.\n\n**Kako radi algoritam:**\n1. **Symbol.toPrimitive hook**: Motor prvo proverava da li objekat ima definisan metod `[Symbol.toPrimitive](hint)`.\n2. **Tri režima rada (hint)**:\n   • **"number"** (aritmetika, unarni `+`): Prvo poziva `.valueOf()`; ako ne vrati primitiv, poziva `.toString()`.\n   • **"string"** (`String(obj)`, template literali): Prvo poziva `.toString()`; ako ne vrati primitiv, poziva `.valueOf()`.\n   • **"default"** (binarni `+`, labava jednakost `==`): Ponaša se isto kao `"number"` za sve objekte (osim za `Date` objekte koji koriste `"string"`).\n3. **Rezultati za uobičajene objekte**:\n   • `[1, 2]` → `valueOf()` vraća niz (objekat) → `toString()` vraća `"1,2"`.\n   • `[]` → `[].toString()` vraća `""` (prazan string).\n   • `{}` → `{}.toString()` vraća `"[object Object]"`.',
          descriptionEn: '`ToPrimitive` is an internal engine abstract operation (ECMA-262 §7.1.1) that converts any object or array into a primitive value (string, number, boolean, symbol) whenever it is evaluated in operations like `+`, `==`, `<`, or explicit wrappers like `String(obj)`.\n\n**The 3-Step Algorithm:**\n1. **Symbol.toPrimitive Hook**: The engine first checks for a custom method at `obj[Symbol.toPrimitive](hint)`.\n2. **The 3 Evaluation Hints**:\n   • **"number"** (arithmetic, unary `+`): Calls `.valueOf()` first; if it returns a non-primitive object, falls back to `.toString()`.\n   • **"string"** (`String(obj)`, template strings): Calls `.toString()` first; if it returns a non-primitive object, falls back to `.valueOf()`.\n   • **"default"** (binary `+`, loose equality `==`): Behaves exactly like `"number"` for almost all objects (except `Date` objects, which default to `"string"`).\n3. **Default Behavior on Arrays & Objects**:\n   • `[1, 2]` → `.valueOf()` returns `[1, 2]` (object, rejected) → `.toString()` returns `"1,2"` (primitive string accepted).\n   • `[]` → `.toString()` returns `""` (empty string).\n   • `{}` → `.toString()` returns `"[object Object]"`.',
          codeSnippet: '// 1. Arrays serialize via toString() -> join(",")\n[1, 2].valueOf()     // [1, 2] (still an Object -> engine ignores it)\n[1, 2].toString()    // "1,2" (Primitive String -> accepted!)\n[].toString()        // "" (empty string)\n\n// 2. Plain Objects serialize via toString()\n({}).valueOf()       // {} (Object -> ignored)\n({}).toString()      // "[object Object]" (Primitive String -> accepted!)\n\n// 3. Custom Symbol.toPrimitive overriding\nconst user = {\n  name: "Alex",\n  score: 100,\n  [Symbol.toPrimitive](hint) {\n    if (hint === "number") return this.score;\n    if (hint === "string") return this.name;\n    return `${this.name} (${this.score} pts)`;\n  }\n};\nconsole.log(+user);        // 100 (hint: "number")\nconsole.log(`${user}`);    // "Alex" (hint: "string")\nconsole.log(user + "!");   // "Alex (100 pts)!" (hint: "default")'
        },
        {
          stepNumber: 2,
          title: 'Dvostruka uloga operatora sabiranja (+) i razlika parsiranja',
          titleEn: 'Dual Nature of the Addition (+) Operator & Parsing vs Coercion',
          description: 'Operator `+` ima dve potpuno različite funkcije (dvostruku prirodu): **1) Sabiranje brojeva** i **2) Spajanje (konkatenaciju) stringova**.\n\nPrema ECMA-262 specifikaciji (§13.15.1):\n• Motor prvo poziva `ToPrimitive()` na oba operanda.\n• **Ako je makar jedan operand String**: oba se pretvaraju u stringove (`ToString`) i **spajaju**.\n• **Ako nijedan nije String**: oba se pretvaraju u brojeve (`ToNumber`) i **matematički sabiraju**.\n\n*Razlika u odnosu na unarni operator (+x):* Za razliku od binarnog plusa (`a + b`), **unarni plus** (`+x`) nema ovu dualnost — on bezuslovno forsira numeričku konverziju (`ToNumber`) na svom jedinom operandu (npr. `+"42"` postaje `42`, a `+[]` postaje `0`).\n\n*Parsiranje naspram Striktne Konverzije:* `Number("10px")` vraća `NaN` jer zahteva da ceo string bude broj, dok `parseInt("10px", 10)` lenjo skenira do prvog ne-numeričkog simbola i vraća `10`. Uvek eksplicitno prosledite osnovu `10` (`radix`).',
          descriptionEn: 'The `+` operator has two completely different operational modes (its dual nature): **1) Numeric Addition** and **2) String Concatenation**.\n\nAccording to the ECMA-262 specification (§13.15.1):\n• The engine first runs `ToPrimitive()` on both operands.\n• **If either operand evaluates to a String**: both operands are coerced to strings via `ToString` and **concatenated**.\n• **If neither operand is a String**: both operands are coerced to numbers via `ToNumber` and **mathematically added**.\n\n*Contrast with the Unary Plus (+x) Operator:* Unlike the binary addition operator (`a + b`), the **unary plus** (`+x`) does NOT share this dual nature — it unconditionally invokes `ToNumber` on its single operand (e.g., `+"42"` evaluates to `42` and `+[]` evaluates to `0`).\n\n*Parsing vs Strict Coercion:* `Number("10px")` returns `NaN` because strict `ToNumber` rejects trailing non-digits, whereas `parseInt("10px", 10)` greedily parses digits left-to-right until `"p"` and returns `10`. Always pass explicit radix `10`.',
          codeSnippet: '// Nature 1: String Concatenation (if either side is String)\n"5" + 3              // "53" ("5" + "3")\n"5" + true           // "5true" ("5" + "true")\n[] + {}              // "[object Object]" ("" + "[object Object]")\n\n// Nature 2: Numeric Addition (if neither side is String)\n5 + true             // 6 (5 + 1)\ntrue + false         // 1 (1 + 0)\n\n// Unary Plus (+x) strictly coerces to Number:\n+"5"                 // 5\n+true                // 1\n+[]                  // 0\n\n// Strict Coercion vs Lenient Substring Parsing:\nNumber("10px")       // NaN (strict)\nparseInt("10px", 10) // 10  (lenient parsing, stops at "px")\nNumber("")           // 0\nparseInt("", 10)     // NaN'
        },
        {
          stepNumber: 3,
          title: 'Algoritam labave jednakosti (==) u 11 koraka (ECMA-262 §7.2.14)',
          titleEn: '11-Step Abstract Equality Algorithm (==) — ECMA-262 §7.2.14',
          description: 'Kada JavaScript izvršava `x == y`, motor ne pogađa nasumično, već prolazi kroz tačno **11 definisanih pravila redom**:\n\n1. **Isti tipovi (`Type(x) === Type(y)`)**: Primenjuje striktnu jednakost `x === y`.\n2. **`null == undefined`**: Vraća `true`.\n3. **`undefined == null`**: Vraća `true`.\n4. **`Number == String`**: Pretvara String u Number (`x == ToNumber(y)`).\n5. **`String == Number`**: Pretvara String u Number (`ToNumber(x) == y`).\n6. **`BigInt == String`**: Pretvara String u BigInt (`StringToBigInt(y)`).\n7. **`String == BigInt`**: Pretvara String u BigInt (`StringToBigInt(x)`).\n8. **`Boolean == bilo šta`**: Pretvara Boolean u broj (`ToNumber(x) == y`, gde `true → 1`, `false → 0`).\n9. **`bilo šta == Boolean`**: Pretvara Boolean u broj (`x == ToNumber(y)`).\n10. **`String/Number/BigInt/Symbol == Object`**: Poziva `ToPrimitive(y)` na objektu.\n11. **`Object == String/Number/BigInt/Symbol`**: Poziva `ToPrimitive(x)` na objektu.\n\n*(Ako nijedno pravilo ne odgovara, vraća `false`.)*',
          descriptionEn: 'When JavaScript evaluates `x == y`, the engine executes an ordered decision tree of **11 strict specification rules** (ECMA-262 §7.2.14 `IsLooselyEqual`):\n\n1. **Same Type (`Type(x) === Type(y)`)**: Runs Strict Equality `x === y`.\n2. **`null == undefined`**: Returns `true`.\n3. **`undefined == null`**: Returns `true`.\n4. **`Number == String`**: Coerces String to Number (`x == ToNumber(y)`).\n5. **`String == Number`**: Coerces String to Number (`ToNumber(x) == y`).\n6. **`BigInt == String`**: Coerces String to BigInt (`StringToBigInt(y)`).\n7. **`String == BigInt`**: Coerces String to BigInt (`StringToBigInt(x)`).\n8. **`Boolean == any`**: Coerces Boolean to Number (`ToNumber(x) == y`, where `true → 1`, `false → 0`).\n9. **`any == Boolean`**: Coerces Boolean to Number (`x == ToNumber(y)`).\n10. **`String/Number/BigInt/Symbol == Object`**: Coerces Object via `x == ToPrimitive(y)`.\n11. **`Object == String/Number/BigInt/Symbol`**: Coerces Object via `ToPrimitive(x) == y`.\n\n*(If none match, returns `false`.)*',
          codeSnippet: '// Example: Tracing `[] == ![]` through the 11-step engine algorithm:\n\n// Step A: Unary `!` runs first -> `![]` evaluates to `false`\n// Expression is now: `[] == false`\n\n// Step B: Rule 9 fires (`any == Boolean`) -> ToNumber(false) -> 0\n// Expression is now: `[] == 0`\n\n// Step C: Rule 11 fires (`Object == Number`) -> ToPrimitive([]) -> [].toString() -> ""\n// Expression is now: `"" == 0`\n\n// Step D: Rule 5 fires (`String == Number`) -> ToNumber("") -> 0\n// Expression is now: `0 == 0`\n\n// Step E: Rule 1 fires (`Type(0) === Type(0)`) -> 0 === 0 -> true!\n[] == ![]; // true'
        },
        {
          stepNumber: 4,
        ...
        {
          stepNumber: 5,
          title: 'Apstraktno relaciono poređenje (<, >, <=, >=) — ECMA-262 §7.2.13',
          titleEn: 'Abstract Relational Comparison (<, >, <=, >=) — ECMA-262 §7.2.13',
          description: 'Za razliku od jednakosti `==`, relacioni operatori funkcionišu po drugom algoritmu:\n1. Poziva se `ToPrimitive(hint: "number")` na oba operanda.\n2. **Ako su oba operanda Stringovi**: vrši se **leksikografsko (alfabetsko)** poređenje UTF-16 kodova (`"10" < "9"` je `true` jer karakter `"1"` dolazi pre karaktera `"9"`).\n3. **Ako makar jedan NIJE String**: oba operanda se konvertuju u brojeve preko `ToNumber()` (`"10" < 9` daje `false` jer `10 < 9` je netačno).',
          descriptionEn: 'Unlike equality `==`, relational operators execute under a distinct algorithm:\n1. Invokes `ToPrimitive(hint: "number")` on both operands.\n2. **If BOTH operands are Strings**: performs **lexicographical (alphabetical)** UTF-16 code comparison (`"10" < "9"` is `true` because `"1"` precedes `"9"`).\n3. **If at least one is NOT a String**: both operands are coerced to numbers via `ToNumber()` (`"10" < 9` is `false` because `10 < 9` is false).',
          codeSnippet: '// Both are strings -> Lexicographical comparison\n"10" < "9"        // true ("1" < "9")\n"100" < "20"      // true\n\n// One is number -> Coerced via ToNumber()\n"10" < 9          // false (10 < 9)\n"100" < 20        // false (100 < 20)\n\n// Relational coercion with null and undefined\nnull >= 0         // true (ToNumber(null) -> 0 >= 0)\nundefined >= 0    // false (ToNumber(undefined) -> NaN >= 0 is false)'
        },
        {
          stepNumber: 6,
          title: '4 algoritma jednakosti u JavaScript-u (==, ===, SameValue, SameValueZero)',
          titleEn: 'The 4 Equality Algorithms in JavaScript (==, ===, SameValue, SameValueZero)',
          description: 'ECMAScript definiše 4 različita algoritma za poređenje dve vrednosti:\n1. **IsLooselyEqual (`==`)**: Apstraktna jednakost sa implicitnom konverzijom tipova (11 koraka).\n2. **IsStrictlyEqual (`===`)**: Striktna jednakost bez konverzije. `NaN === NaN` je `false`, a `+0 === -0` je `true`.\n3. **SameValue (`Object.is`)**: Razlikuje `+0` i `-0` (`Object.is(+0, -0)` vraća `false`) i smatra `NaN` jednakim samom sebi (`Object.is(NaN, NaN)` vraća `true`). U IEEE 754 standardu, `-0` postoji da bi se očuvao smer aproksimacije i brzine (npr. `1 / +0 === Infinity`, dok `1 / -0 === -Infinity`).\n4. **SameValueZero**: Koriste ga `Map`, `Set` i `Array.prototype.includes`. Tretira `+0` i `-0` kao jednake, ali takođe smatra da je `NaN` jednak `NaN` (zato `[NaN].includes(NaN)` vraća `true`, dok `[NaN].indexOf(NaN)` koji koristi `===` vraća `-1`).',
          descriptionEn: 'ECMAScript formalizes 4 distinct equality algorithms:\n1. **IsLooselyEqual (`==`)**: Abstract equality with implicit type conversion (11 steps).\n2. **IsStrictlyEqual (`===`)**: Strict equality without coercion. `NaN === NaN` is `false`, and `+0 === -0` is `true`.\n3. **SameValue (`Object.is`)**: Distinguishes `+0` from `-0` (`Object.is(+0, -0)` returns `false`) and matches `NaN` to `NaN` (`Object.is(NaN, NaN)` returns `true`). In IEEE 754 arithmetic, `-0` preserves mathematical sign of approach (e.g. `1 / +0 === Infinity`, whereas `1 / -0 === -Infinity`).\n4. **SameValueZero**: Used internally by `Map`, `Set`, and `Array.prototype.includes`. Treats `+0` and `-0` as identical, but also considers `NaN` equal to `NaN` (which is why `[NaN].includes(NaN)` returns `true`, while `[NaN].indexOf(NaN)` using `===` returns `-1`).',
          codeSnippet: '// 1. Strict Equality (===)\nNaN === NaN                 // false\n+0 === -0                   // true\n\n// 2. SameValue (Object.is) & Signed Zeros:\nObject.is(NaN, NaN)         // true\nObject.is(+0, -0)           // false\n1 / +0                      // Infinity\n1 / -0                      // -Infinity\n\n// 3. SameValueZero (Map, Set, Array.includes)\n[NaN].indexOf(NaN)          // -1 (uses ===)\n[NaN].includes(NaN)         // true (uses SameValueZero)\nnew Set([+0, -0]).size      // 1 (uses SameValueZero)'
        }
      ]
    },
    pitfalls: [
