...
export const QUIZ_QUESTIONS: QuizQuestion[] = [
  {
    ...
    titleEn: 'Transitivity of Loose Equality (==)',
...
console.log("0" == []);`,
    ...
    explanationEn: 'Expression `0 == "0"` is `true` (string converts to number `0`). Expression `0 == []` is `true` (`[]` converts to `""` then `0`). BUT `"0" == []` compares String and Object: `[]` becomes `""`, and `"0" == ""` is `false`! Loose equality in JavaScript is NOT transitive.',
    ecmaRule: 'ECMA-262 §7.2.14 Abstract Equality Comparison',
    ecmaRuleEn: 'ECMA-262 §7.2.14 Abstract Equality Comparison',
...
console.log(numbers);`,
    ...
    explanationEn: 'By default, `Array.prototype.sort()` converts elements to strings and compares their UTF-16 code units lexicographically. Thus string `"10"` alphabetically precedes `"2"`. For numeric sorting, always provide a comparator function `(a, b) => a - b`.',
    ecmaRule: 'ECMA-262 §23.1.3.30 Array.prototype.sort',
    ecmaRuleEn: 'ECMA-262 §23.1.3.30 Array.prototype.sort',
    difficulty: 'Beginner',
  ...
  },
  {
    ...
    titleEn: 'Double Typeof Operator',
    ...
    explanationEn: 'First `typeof 1` evaluates to string `"number"`. Next `typeof "number"` evaluates to `"string"` because the return value of any `typeof` operator is always a string!',
    ecmaRule: 'ECMA-262 §13.5.3 The typeof Operator vraća string primitiv',
    ecmaRuleEn: 'ECMA-262 §13.5.3 The typeof Operator returns string primitive',
    difficulty: 'Beginner',
    category: 'type-coercion'
  },
  {
    ...
    titleEn: 'Relational vs Loose Equality with Null',
...
console.log(null >= 0);`,
    ...
    explanationEn: 'Relational operator `>=` coerces `null` via `Number(null)` to `0` (`0 >= 0` is `true`). But `==` uses equality rules where `null` only loosely equals `undefined` and `null` (`null == 0` is `false`)!',
    ecmaRule: 'ECMA-262 §7.2.13 Abstract Relational naspram §7.2.14 Abstract Equality',
    ecmaRuleEn: 'ECMA-262 §7.2.13 Abstract Relational vs §7.2.14 Abstract Equality',
    difficulty: 'WTJS Quirks',
    category: 'type-coercion'
  },
  {
    ...
    titleEn: 'Function Hoisting vs Variable Shadowing',
    codeSnippet: `var a = 1;
function test() {
  a = 10;
  return;
...
console.log(a);`,
    ...
    explanationEn: 'Inside `test()`, `function a() {}` declaration is hoisted to top of function scope creating local binding `a`. Assignment `a = 10` modifies that local binding, leaving the outer global `a` untouched at `1`!',
    ecmaRule: 'ECMA-262 §10.2.1 FunctionDeclarationInstantiation hoisting pravilo',
    ecmaRuleEn: 'ECMA-262 §10.2.1 FunctionDeclarationInstantiation hoisting rule',
    difficulty: 'Advanced',
  ...
  },
  {
    ...
    titleEn: 'Sparse Arrays and Array.map()',
...
console.log(result.length, result[1]);`,
    ...
    explanationEn: 'The `delete arr[1]` operator deletes the property without re-indexing, leaving an empty hole. `Array.prototype.map` skips non-existent properties per ECMAScript spec, yielding `[2, <empty>, 6]` with length `3`, where `result[1]` evaluates to `undefined`.',
    ecmaRule: 'ECMA-262 §23.1.3.19 Array.prototype.map (HasProperty provera)',
    ecmaRuleEn: 'ECMA-262 §23.1.3.19 Array.prototype.map (HasProperty check)',
  ...
  },
  {
    ...
    titleEn: 'Math.min and Math.max Extremes',
    ...
    explanationEn: '`Math.min()` without arguments returns `+Infinity` (identity element for minimum). `Math.max()` returns `-Infinity`. Because `+Infinity > -Infinity`, the expression produces `true`!',
    ecmaRule: 'ECMA-262 §21.2.2.24 & §21.2.2.25 Math svojstva',
    ecmaRuleEn: 'ECMA-262 §21.2.2.24 & §21.2.2.25 Math properties',
    difficulty: 'WTJS Quirks',
  ...
  },
  {
    ...
    titleEn: 'Array Addition Operator',
    ...
    explanationEn: 'Arrays do not support numeric addition. Operator `+` triggers `ToPrimitive()` which calls `.toString()` on both: `[1, 2].toString()` yields `"1,2"`, `[3, 4].toString()` yields `"3,4"`. Concatenation results in `"1,23,4"`.',
    ecmaRule: 'ECMA-262 §13.15.3 ApplyStringOrNumericBinaryOperator',
    ecmaRuleEn: 'ECMA-262 §13.15.3 ApplyStringOrNumericBinaryOperator',
    difficulty: 'Intermediate',
    category: 'type-coercion'
  },
  {
    ...
    titleEn: 'Object Key Stringification',
    codeSnippet: `const a = {};
const b = { key: 'b' };
const c = { key: 'c' };
...
console.log(a[b]);`,
    ...
    explanationEn: 'Property keys on plain JS objects must be String or Symbol. Both `b` and `c` stringify to `"[object Object]"`. Therefore both `a[b]` and `a[c]` write to `a["[object Object]"]`, so the second assignment `456` overwrites the first.',
    ecmaRule: 'ECMA-262 §7.1.17 ToPropertyKey',
    ecmaRuleEn: 'ECMA-262 §7.1.17 ToPropertyKey',
    difficulty: 'Intermediate',
    category: 'arrays-objects'
  },
  {
    ...
    titleEn: 'Automatic Semicolon Insertion (ASI) on Return',
...
console.log(foo());`,
    ...
    explanationEn: 'JavaScript automatically inserts a semicolon after `return` due to the newline preceding `{`. The code becomes `return;`, leaving `{ name: "JS" }` as unreachable block code. The function returns `undefined`.',
    ecmaRule: 'ECMA-262 §12.9.1 Pravila automatskog umetanja tačka-zapete (ASI)',
    ecmaRuleEn: 'ECMA-262 §12.9.1 Automatic Semicolon Insertion (ASI) rules',
    difficulty: 'Beginner',
    category: 'syntax-asi'
  },
  {
...
console.log('D');`,
    ...
    explanationEn: 'Synchronous logs `\'A\'` and `\'D\'` run immediately on Call Stack. Then Microtask queue (`Promise \'C\'`) completely drains before processing Macrotask queue (`setTimeout \'B\'`). Final order: `A`, `D`, `C`, `B`.',
    ecmaRule: 'HTML Event Loop: Microtask red se prazni pre sledeće iteracije glavnog task loop-a',
    ecmaRuleEn: 'HTML Event Loop: Microtask queue drains before next task execution',
    difficulty: 'Intermediate',
    category: 'event-loop'
  },
  {
    ...
    titleEn: 'Unary Plus and Boolean Arithmetic',
    ...
    explanationEn: 'Unary `+true` evaluates to `1`. `!!"false"` (non-empty string is truthy) yields `true` (coerced to `1` during addition). `+[]` coerces `[]` to `""` then `0`. Sum: `1 + 1 + 0 = 2`.',
    ecmaRule: 'ECMA-262 §13.5.6 Unary + Operator & §7.1.1 ToNumber',
    ecmaRuleEn: 'ECMA-262 §13.5.6 Unary + Operator & §7.1.1 ToNumber',
    difficulty: 'WTJS Quirks',
    category: 'type-coercion'
  }
];
